  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-700 space-y-12 md:space-y-20">
      ...
      <section className="relative pt-6 md:pt-12 pb-4 text-center px-2">
        <div className="absolute top-0 left-1/2 -translate-x-1/2 w-full max-w-[800px] h-[400px] -z-10 bg-indigo-500/5 blur-[100px] rounded-full pointer-events-none" />
        ...
        <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-white/[0.03] border border-white/[0.05] mb-6 animate-in slide-in-from-top-4 duration-700">
           ...
           <span className="text-[8px] md:text-[10px] font-black uppercase tracking-widest text-slate-400">Gemini 3 Pro Visual Engine Active</span>
        </div>

        <h2 className="text-3xl md:text-6xl font-black tracking-tighter mb-4 md:mb-6 bg-clip-text text-transparent bg-gradient-to-br from-white via-indigo-100 to-slate-400 leading-[1.1]">
          Studio Kreatif AI <br className="hidden md:block" /> Masa Depan
        </h2>
        
        <p className="text-slate-400 text-sm md:text-lg max-w-xl mx-auto mb-8 md:mb-12 leading-relaxed font-medium px-4">
          Wujudkan segala kebutuhan visual, jualan, dan konten media sosial Anda dengan kualitas studio profesional sekejap mata.
        </p>
        
        <div className="flex flex-col sm:flex-row justify-center gap-3 md:gap-5 px-4">
          <button 
            onClick={() => onNavigate('text2img')}
            className="group relative bg-indigo-600 hover:bg-indigo-500 text-white px-8 py-4 rounded-xl font-black text-[10px] uppercase tracking-widest inline-flex items-center justify-center gap-3 shadow-xl shadow-indigo-600/20 transition-all active:scale-95 overflow-hidden"
          >
            <Rocket className="w-4 h-4" /> Mulai Berkreasi
          </button>
          <button 
          ...
          >
            Tulis Iklan <ArrowRight className="w-4 h-4" />
          </button>
        </div>
      </section>
      ...
      <section className="bg-slate-900/40 border border-white/5 rounded-[2.5rem] p-8 md:p-12 space-y-10">
         <div className="text-center space-y-2">
            <h3 className="text-xs font-black text-indigo-400 uppercase tracking-[0.3em]">Panduan Penggunaan</h3>
         ...
         <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              ...
              <div key={i} className="space-y-4 text-center md:text-left group">
                 <div className="w-12 h-12 bg-white/5 rounded-2xl flex items-center justify-center mx-auto md:mx-0 group-hover:scale-110 transition-transform group-hover:bg-indigo-600/20">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
            {group.items.map((item) => (
              <div 
                ...
                className="group relative bg-[#0a0f1d]/40 border border-white/[0.03] p-5 md:p-7 rounded-2xl md:rounded-[2rem] cursor-pointer hover:border-indigo-500/20 hover:bg-white/[0.05] transition-all duration-300 flex flex-col"
              >
                ...
                <div className="flex-1">
                  <h3 className="text-sm md:text-lg font-bold mb-1 md:mb-2 text-slate-100 group-hover:text-indigo-400 transition-colors">{item.label}</h3>
